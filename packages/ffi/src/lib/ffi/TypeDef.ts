...
import {TypeSpecWithAlignment} from './types/TypeSpecWithAlignment'
import {TypeSpec} from './types/TypeSpec'
import {ArrayHint} from './types/ArrayHint'
...
export class TypeDef {
    ...
    public static union(name: string, def: Record<string, TypeSpecWithAlignment>): TypeInfo {
        return As(koffi.union(name, def))
    }
    ...
    public static struct(name: string, def: Record<string, TypeSpecWithAlignment>): TypeInfo {
        return As(koffi.struct(name, def))
    }
    ...
    public static array(ref: TypeSpec, len: number, hint?: ArrayHint | null): TypeInfo {
        return As(koffi.array(ref, len, hint))
    }
    ...
    public static pointer(nameOrRef: string, ref?: TypeSpec): TypeInfo {
        return As(ref ? koffi.pointer(nameOrRef, ref) : koffi.pointer(nameOrRef))
    }
    ...
    public static opaque(name?: string): TypeInfo {
        return As(name ? koffi.opaque(name) : koffi.opaque())
    }
    ...
    public static proto(conventionOrDefinition: string, nameOrResult?: string | TypeSpec, resultOrArgs?: TypeSpec | TypeSpec[], args?: TypeSpec[]): TypeInfo {
        switch (arguments.length) {
            case 3:
                return As(koffi.proto(conventionOrDefinition, As<TypeSpec>(nameOrResult), As<TypeSpec[]>(resultOrArgs)))
            case 4:
                return As(koffi.proto(conventionOrDefinition, As<string>(nameOrResult), As<TypeSpec>(resultOrArgs), As<TypeSpec[]>(args)))
            default:
                return As(koffi.proto(conventionOrDefinition))
        }
    }
}
